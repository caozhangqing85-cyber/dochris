...
> **状态说明（2026-09-05，DOC-01）**：本页是历史改进计划记录。表中四个阶段均已落地；
> 其中“第 1 周”虽在正文标为“下一步”，实际已于 2026-06-10 完成（见下表），以本横幅为准。
> 评测基线与收益门槛的当前约定见 **[RELEASE.md](RELEASE.md)** 与 `eval/` 模块文档。
...
1. ~~**第 0 周（前置任务，必须先完成）**：统一查询链路 LLM 客户端（消除同步/异步分裂）、实现统一候选模型 `RetrievalCandidate`（含显式归一化分数语义）、统一 VectorStore 接口（Protocol 对齐 ABC）。这些是后续所有方案的架构基础。~~ **✅ 已完成（2026-06-09）**
2. **第 1 周**：补 Reranker 和 RAG 评估。前者直接提升上下文质量，后者建立可验证的改进闭环。 **✅ 已完成（见下方进度表）**
...
| 阶段 | 状态 | 完成日期 | 关键产出 |
|------|------|----------|----------|
| 第 0 周：前置任务 | ✅ 完成 | 2026-06-09 | `rag/schemas.py`（RetrievalCandidate + SourceRef + 归一化）、`retrieve_candidates()`、`build_answer_context()`、VectorStore Protocol 对齐、查询链路 async-first、`_env_mapping` + 7 个 RAG 配置项 |
| 第 1 周：Reranker + RAG 评估 | ✅ 完成 | 2026-06-10 | `rag/reranker/`（BaseReranker ABC + CrossEncoderReranker + IdentityReranker + factory）、`rerank_candidates()` 集成、API `rerank` 参数、CLI `--rerank`、`eval/` 模块（schemas + datasets + rag_metrics + runner）、JSONL golden set 支持、recall@k/precision@k/MRR/NDCG 指标、JSON + Markdown 报告生成 |
| 第 2 周：可观测性 + 流式输出 | ✅ 完成 | 2026-06-10 | `observability/`（tracing + metrics + cost + middleware）、Prometheus Counter/Histogram（query/LLM/retrieval/rerank/cache）、`/metrics` 端点、trace_id 中间件 + API 响应头、LLMProvider usage/latency/cost 自动记录、`api/sse.py`（StrEnum 事件名 + 版本化 + 心跳）、SSE 流式重构 |
| 第 3 周：语义分块 + 优化 | ✅ 完成 | 2026-06-14 | `rag/chunking/`（BaseChunker ABC + StructureChunker + RecursiveChunker + SemanticChunker + factory）、raw chunks indexing（INDEX_RAW_CHUNKS 开关 + chunks collection）、store 实例缓存复用、6 个新配置项、26 个单元测试 |
...
### 第 1 周：Reranker + RAG 评估（5 天） ✅ 已完成（2026-06-10）

Day 1-2：✅

- [x] 实现 BGE / SentenceTransformers reranker（`sentence_transformers.CrossEncoder` 作为默认轻量实现，BGE 作为可选）。
- [x] `kb query` 和 `/api/v1/query` 支持 `rerank=true`。
- [x] 加 reranker 单测（15 个测试）和 lint 通过。

Day 3-4：✅

- [x] 新增 RAG eval schema、dataset loader、基础 retrieval metrics。
- [x] 支持 golden JSONL（加载/保存/自动生成）。
- [x] 检索指标：recall@k、precision@k、MRR、NDCG（25 个测试全通过）。
- [x] 生成 JSON + Markdown 评估报告。
...
### 第 2 周：可观测性 + 流式输出（6-7 天） ✅ 已完成（2026-06-10）

Day 1-2：✅

- [x] 新增 observability manager（ObservabilityManager 门面 + 单例）。
- [x] Prometheus counters/histograms（query/LLM/retrieval/rerank/cache 5 类指标）。
- [x] `/metrics` 端点（PROMETHEUS_ENABLED 开关控制）。

Day 3-4：✅

- [x] LLM 调用自动记录 usage/latency/cost（openai_compat.py 内埋点）。
- [x] response 增加 trace_id（X-Trace-Id 响应头 + QueryResponse.trace_id 字段）。
- [x] middleware 执行顺序：CORS → TracingMiddleware → auth → route handler。
- [x] 验证默认不记录敏感全文（ObservabilityManager.enabled=false 时零开销）。

Day 5-6：✅

- [x] SSE 事件类型化（QueryStreamEventName StrEnum + v=1 版本字段）。
- [x] 增加 ping、error、done、versioned events（sse.py 编码器）。
...
- [x] 新增 chunking 抽象（`rag/chunking/base.py`：BaseChunker ABC + ChunkMetadata + DocumentChunk）。
- [x] StructureChunker（包装现有 structure_aware_split，补充字符定位）。
- [x] RecursiveChunker（token-aware 递归分块，借鉴 LangChain RecursiveCharacterTextSplitter）。
- [x] SemanticChunker（基于 embedding 相邻距离断点，支持外部注入 embedding_func 和降级）。
- [x] factory create_chunker（三策略注册 + 未知策略异常）。

Day 3：✅

- [x] raw chunks indexing 集成（`_index_raw_chunks()` + INDEX_RAW_CHUNKS 开关）。
- [x] 新增 chunks collection（metadata 含 source/section/start_char/end_char/strategy/trust_level）。
- [x] store 实例提升为 CompilerWorker 属性（`_vector_store` 缓存复用，避免重复初始化 embedding 模型）。

Day 4：✅

- [x] 查询侧自动支持 chunks collection（`_vector_search_with_store` 遍历 list_collections，新增 collection 透明纳入）。
- [x] chunking 配置项（chunk_size_chars/tokens、overlap、semantic_breakpoint_percentile，通过 _env_mapping 注册）。
- [x] 缓存策略：chunk indexing 独立于编译缓存，缓存命中时可通过 `kb recompile` 重建。

Day 5：✅

- [x] 26 个单元测试全通过（数据结构 + 三策略 + factory + 集成对比）。
- [x] Lint 全通过。
- [x] 文档更新（ROADMAP + RAG_IMPROVEMENT_PLAN）。
