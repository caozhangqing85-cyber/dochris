...
class TestCliReviewPromote:
    def test_promote_gate_fail(self, tmp_path):
        ...
        with (
            patch("dochris.settings.get_default_workspace", return_value=tmp_path),
            patch(
                "dochris.quality.quality_gate.quality_gate",
                return_value={"passed": False, "reason": "low"},
            ),
        ):
...
class TestCliReviewQuality:
    ...
    def test_quality_pollution_yes(self, tmp_path):
        ...
        with (
            patch("dochris.settings.get_default_workspace", return_value=tmp_path),
            patch(
                "dochris.quality.quality_gate.check_pollution",
                return_value={"polluted": True, "details": "bad"},
            ),
        ):
    ...
    def test_quality_src_id_pass(self, tmp_path):
        ...
        with (
            ...
            patch(
                "dochris.quality.quality_gate.quality_gate",
                return_value={"passed": True, "src_id": "S1", "title": "T"},
            ),
        ):
            assert cmd_quality(args) == 0
    ...
    def test_quality_scan_wiki(self, tmp_path):
        ...
        with (
            ...
            patch(
                ...
                return_value={"wiki_summaries": 5, "wiki_concepts": 3, "wiki_total": 8},
            ),
        ):
            assert cmd_quality(args) == 0
