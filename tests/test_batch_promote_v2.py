    ...
    def test_limit_truncates_candidates(self, tmp_path):
        ...
        with patch("dochris.admin.batch_promote.get_all_manifests", return_value=manifests):
            with patch("dochris.admin.batch_promote.promote_to_wiki") as mock_promote:
                ...
                batch_promote_to_wiki(tmp_path, limit=2)

        # limit=2 截断到 2 个候选
        assert mock_promote.call_count == 2

    def test_candidates_overflow_20_dry_run(self, tmp_path, caplog):
        ...
        with patch("dochris.admin.batch_promote.get_all_manifests", return_value=manifests):
            with caplog.at_level(logging.INFO, logger="dochris.admin.batch_promote"):
                batch_promote_to_wiki(tmp_path, limit=0, dry_run=True)

        assert "还有" in caplog.text
